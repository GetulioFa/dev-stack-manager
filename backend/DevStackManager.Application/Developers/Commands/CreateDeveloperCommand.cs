...
public sealed class CreateDeveloperCommandHandler(
    IDeveloperRepository developerRepository,
    ICityRepository cityRepository,
    ILanguageRepository languageRepository,
    IUnitOfWork unitOfWork
) : IRequestHandler<CreateDeveloperCommand, Result<DeveloperDto>>
{
    public async Task<Result<DeveloperDto>> Handle(CreateDeveloperCommand request, CancellationToken cancellationToken)
    {
        var existing = await developerRepository.GetByEmailAsync(request.Email, cancellationToken);
        ...
        var result = Developer.Create(request.Name, request.Email, request.Seniority, request.CityId, languageIds);
        if (result.IsFailure)
            return Result<DeveloperDto>.Failure(result.Error!);

        await developerRepository.AddAsync(result.Value, cancellationToken);
        await unitOfWork.SaveChangesAsync(cancellationToken);

        return Result<DeveloperDto>.Success(MapToDto(result.Value, city, languages));
    }
